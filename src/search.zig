...
fn Search(Ctrl: type) type {
    return struct {
        game: *Game,
        ctrl: Ctrl,

        fn search2(self: *const @This(), pv: anytype, alpha: Score, beta: Score, ply: u32, depth: i32, comptime mode: SearchMode) SearchError!Score {
            return if (mode != .quiescence and depth <= 0)
                try self.search(pv, alpha, beta, ply, depth, .quiescence)
            else if (mode == .firstply)
                try self.search(pv, alpha, beta, ply, depth, .normal)
            else
                try self.search(pv, alpha, beta, ply, depth, mode);
        }

        fn search(self: *const @This(), pv: anytype, alpha: Score, beta: Score, ply: u32, depth_arg: i32, comptime mode: SearchMode) SearchError!Score {
            var depth = depth_arg;
            ...
            try self.ctrl.checkHardTermination(mode, depth);

            // Are we on a PV node?
            const is_pv_node = beta != alpha + 1;

            const tte = self.game.ttLoad();
            ...
            const first_static_eval = eval.eval(self.game);
            ...
            const is_in_check = self.game.board.isInCheck();
            ...
            if (!is_in_check and (mode == .normal or mode == .nullmove) and depth > 2 and !self.game.prevMove().isNone()) {
                const old_state = self.game.moveNull();
                const nws_reduction = 4 + @divFloor(depth, 6);
                const null_score = -try self.search2(line.Null{}, -beta, -beta +| 1, ply + 1, depth - nws_reduction, .normal);
                self.game.unmoveNull(old_state);
                if (null_score >= beta) {
                    if (mode == .nullmove) {
                    ...
                    return self.search(line.Null{}, alpha, beta, ply, depth - nmr_reduction, .nullmove);
                }
            }

            var moves = MoveList{};
            switch (mode) {
                .firstply, .normal, .nullmove => moves.generateMoves(&self.game.board, .any),
                .quiescence => moves.generateMoves(&self.game.board, .captures_only),
            }
            self.game.sortMoves(&moves, best_move);
            ...
            for (0..moves.size) |i| {
                ...
                const old_state = self.game.move(m);
                defer self.game.unmove(m, old_state);
                if (self.game.board.isValid()) {
                    // Late Move Pruning
                    ...
                    const child_score = blk: {
                        if (self.game.board.isRepeatedPosition() or self.game.board.is50MoveExpired()) break :blk 0;
                        ...
                        if (mode != .quiescence and quiets_visited > 2 and depth > 2) {
                            ...
                            if (r > 1) {
                                const lmr_score = -try self.search2(&child_pv, -a - 1, -a, ply + 1, depth - r, mode);
                        ...
                        if (mode != .quiescence and moves_visited != 0 and is_pv_node) {
                            const scout_score = -try self.search2(&child_pv, -a - 1, -a, ply + 1, depth - 1, mode);
                            if (scout_score <= a or scout_score >= beta) break :blk scout_score;
                        }

                        break :blk -try self.search2(&child_pv, -beta, -a, ply + 1, depth - 1, mode);
                    };

                    self.ctrl.nodeVisited();
                    moves_visited += 1;
            ...
            if (best_score >= beta and mode != .quiescence) {
                self.game.recordHistory(depth, &moves, best_i);
            }
        ...
        fn forDepth(self: *const @This(), pv: anytype, depth: i32, prev_score: Score) SearchError!Score {
            ...
            if (depth > 3) {
                ...
                while (true) : (delta *= 2) {
                    score = try self.search(pv, lower, upper, 0, depth, .firstply);
                    if (score <= lower) {
            ...
            // Full window
            return try self.search(pv, min_window, max_window, 0, depth, .firstply);
        }

        fn go(self: *const @This(), output: anytype, pv: anytype) !Score {
            var depth: i32 = 1;
            var score: Score = undefined;
            var current_pv = pv.new();
            while (depth < common.max_search_ply) : (depth += 1) {
                score = self.forDepth(&current_pv, depth, score) catch {
                    try output.print("info depth {} score cp {} {} pv {} string [search terminated]\n", .{ depth, score, self.ctrl, pv });
                    break;
                };
                pv.copyFrom(&current_pv);
                try output.print("info depth {} score cp {} {} pv {}\n", .{ depth, score, self.ctrl, pv });
                if (self.ctrl.checkSoftTermination(depth)) break;
            }
            return score;
        }
    };
...
pub fn go(output: anytype, game: *Game, ctrl: anytype, pv: anytype) !Score {
    ...
    const search = Search(@TypeOf(ctrl)){ .game = game, .ctrl = ctrl };
    return search.go(output, pv);
}
